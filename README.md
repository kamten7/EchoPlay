# My Video Platform - 视频分享平台（后端）

> **一个功能完备的全栈视频分享平台后端，灵感来源于 Bilibili，支持视频上传转码、弹幕互动、用户社交与管理后台。**  
> 前端完整代码位于另一个仓库：[My Video Platform_front](https://github.com/kamten7)。 
...
## 项目简介
My Video Platform 是一个前后端分离的视频分享平台，提供完整的视频上传、转码、播放、弹幕互动、用户社交以及后台管理能力。平台参考了 Bilibili 的核心交互模式（弹幕、投币、收藏等），采用现代 Web 技术栈构建，适用于学习视频平台开发、展示全栈能力或搭建轻量级视频社区。
## 项目背景与动机
视频分享平台是互联网最复杂也最具代表性的应用场景之一，涉及**大文件上传、媒体转码、实时评论（弹幕）、社交互动、内容审核**等多个技术领域。创建 My Video Platform 的目标是：
...
| **暗黑模式** | CSS 变量驱动的暗色/亮色主题切换，跟随系统偏好 |
...
│                         浏览器                                   │
│  ┌──────────────────────┐  ┌─────────────────────────────────┐  │
│  │ My Video Platform_front_web  │  │ My Video Platform_front_admin       │  │
...
┌───────────┼───────────────────────────┼─────────────────────────┐
│  ┌────────▼───────────┐  ┌───────────▼───────────────────┐      │
│  │ My Video Platform-web        │  │ My Video Platform-admin              │  │
...
| **Axios** | HTTP 客户端 | 拦截器机制便于统一处理 Token 刷新和错误提交 |
...
创建名为 `my_video_platform` 的 MySQL 数据库，并执行建表脚本（位于 `my_video_platform-common/src/main/resources/` 目录下的 SQL 文件，如项目暂缺则需根据实体类手动生成 DDL）。
### 3. 启动后端
...
> 前端代码位于独立仓库 [My Video Platform_front](https://github.com/kamten7)，请克隆前端仓库后按以下步骤操作。
```bash
# 用户端前端（端口 3001）
cd My_video_platform_front/my_video_platform_front_web
npm install
npm run dev

# 管理端前端（端口 3000，新终端）
cd My_video_platform_front/my_video_platform_front_admin
npm install
npm run dev
```
...
curl -X POST http://localhost:7071/danmu/postDanmu \
  -d "videoId=xxx&fileId=xxx&text=Hello My Video Platform&mode=0&color=%23ffffff&time=10"
```
...
# 运行特定模块测试
mvn test -pl my_video_platform-web
```

建议增加的测试覆盖范围：
- **单元测试**：Service 层业务逻辑（JUnit 5 + Mockito）
- **集成测试**：Controller / API 接口（Spring MockMvc）
- **数据访问测试**：MyBatis Mapper 查询（@MybatisTest / Testcontainers）
### 前端测试

```bash
cd My_video_platform_front/my_video_platform_front_web
npm run test         # Vitest 单元测试（如已配置）
npm run test:e2e     # Playwright/Cypress E2E（如已配置）
```
...
│   └── src/main/java/com/myvideoplatform/web/
...
│       └── MyVideoPlatformWebRunApplication.java   # 启动类
│
├── my_video_platform-admin/                         # 管理端 API（端口 7070）
│   └── src/main/java/com/myvideoplatform/admin/
│       ├── controller/                 # 管理员控制器
...
│       ├── config/                          # Web 配置
│       ├── interceptor/                     # 登录拦截器
│       └── MyVideoPlatformAdminRunApplication.java # 启动类
│
└── my_video_platform-common/                        # 共享模块
    └── src/main/java/com/myvideoplatform/
        ├── config/                     # 公共配置
...
> **前端项目**：前端代码（Vue 3 + TypeScript + Vite）位于另一个仓库 `My_video_platform_front`，包含用户端（端口 3001）和管理端（端口 3000）。详见前端仓库的 README。
## 贡献指南
