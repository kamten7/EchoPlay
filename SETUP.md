...
Kilikili 是一个全栈视频分享平台后端，灵感来源于 Bilibili。项目采用 Spring Boot + MyBatis + MySQL + Redis 技术栈，支持视频分片上传、FFmpeg 异步转码、弹幕系统、用户社交等完整功能。

**GitHub 仓库**: https://github.com/kamten7/My_video_platform.git

**前端仓库**: 前端代码（Vue 3 + TypeScript + Vite）在独立仓库 kilikili_front 中。
...
| Docker Compose | 内置于 Docker Desktop | 自动包含 |
| JDK 1.8 | — | 仅本地开发时需要 |
| Maven 3.6+ | — | 仅本地编译时需要 |
...
#   WINDOWS_PROJECT_DIR    — 文件存储目录（Windows 路径，如 D:/webser/kilikili）
...
```powershell
# 在 PowerShell 中执行
mkdir D:\webser\kilikili\video\upload -Force
mkdir D:\webser\kilikili\video\original -Force
mkdir logs -Force
```
...
编译成功后会生成：
- `kilikili-web/target/kilikili-web-1.0.jar`
- `kilikili-admin/target/kilikili-admin-1.0.jar`
...
docker-compose logs -f kilikili-web    # 用户端 API 日志
docker-compose logs -f kilikili-admin  # 管理端 API 日志
```
...
# 重启某个服务
docker-compose restart kilikili-web

# 进入 MySQL 容器
docker exec -it kilikili-mysql mysql -uroot -p

# 进入 Redis 容器
docker exec -it kilikili-redis redis-cli -a <REDIS_PASSWORD>
...
```sql
CREATE DATABASE kilikili DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```
...
project:
  folder: d:/webser/kilikili/   # 文件存储目录
...
3. 重启服务：
```powershell
docker-compose restart kilikili-web kilikili-admin
```
...
### 5. 文件存储目录找不到
- 确保在 `.env` 中设置的 `WINDOWS_PROJECT_DIR` 目录存在
